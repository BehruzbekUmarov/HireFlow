...
public class DeleteCvCommandHandler : IRequestHandler<DeleteCvCommand>
{
	...
	public DeleteCvCommandHandler(IAppDbContext db, ICurrentUser currentUser)
	{
		_db = db;
	...
	public async Task Handle(
		DeleteCvCommand command, CancellationToken cancellationToken)
	{
		...
		var usedInActiveApplications = await _db.JobApplications
			.AnyAsync(a => a.CvId == command.CvId &&
